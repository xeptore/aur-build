...
wget -q "https://github.com/gohugoio/hugo/releases/download/v${VERSION}/hugo_${VERSION}_checksums.txt" -O checksums.txt

sha256sums_x86_64="$(grep "hugo_extended_${VERSION}_linux-amd64.tar.gz" checksums.txt | cut -d ' ' -f 1)"
sha256sums_aarch64="$(grep "hugo_extended_${VERSION}_linux-arm64.tar.gz" checksums.txt | cut -d ' ' -f 1)"
